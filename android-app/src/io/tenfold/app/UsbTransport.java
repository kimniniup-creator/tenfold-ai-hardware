...
final class UsbTransport {
  ...
  void send(JSONObject frame){
    final String wire=frame.toString()+"\n";writes.execute(()->{UsbDeviceConnection c=connection;UsbEndpoint ep=output;if(c==null||ep==null){postError("USB_NOT_CONNECTED");return;}byte[] bytes=wire.getBytes(StandardCharsets.UTF_8);int count=c.bulkTransfer(ep,bytes,bytes.length,2000);if(count!=bytes.length)postError("USB_WRITE_FAILED");else postState("USB_SENT_WAITING_ACK");});
  }
  private void readLoop(int session,UsbDeviceConnection localConnection,UsbEndpoint localInput){ByteArrayOutputStream line=new ByteArrayOutputStream();byte[] buffer=new byte[512];boolean discarding=false;while(reading&&session==generation){int count=localConnection.bulkTransfer(localInput,buffer,buffer.length,500);if(count<0)continue;for(int i=0;i<count;i++){int value=buffer[i]&0xFF;if(value=='\n'){if(!discarding&&!line.toString().isEmpty())decode(line.toByteArray());line.reset();discarding=false;}else if(value!='\r'&&!discarding){if(line.size()>=4096){line.reset();discarding=true;postError("FRAME_TOO_LARGE");}else line.write(value);}}}if(session==generation)postState("USB_DISCONNECTED");}
  private void decode(byte[] bytes){try{JSONObject frame=new JSONObject(new String(bytes,StandardCharsets.UTF_8));main.post(()->listener.onFrame(frame));}catch(Exception error){postError("INVALID_JSON_FRAME");}}
