...
final class DeviceProtocol {
  ...
  static JSONObject eventAck(SharedPreferences p,int seq) {JSONObject value=new JSONObject();put(value,"type","event_ack");put(value,"command_id",p.getString("command_id",""));put(value,"cycle_id",p.getString("cycle_id",""));put(value,"revision",p.getInt("revision",1));put(value,"seq",seq);put(value,"persisted",true);return value;}
