...
final class CycleState {
  ...
  boolean startRealFromConfirmedCard(){
    SharedPreferences.Editor edit=prefs.edit();for(int day=1;day<=10;day++)edit.remove("day_"+day+"_status").remove("day_"+day+"_date").remove("day_"+day+"_action").remove("day_"+day+"_done").remove("day_"+day+"_note").remove("day_"+day+"_next");
    return commit(edit.putString("mode",MODE_USB).putInt("day_index",1).putBoolean("completed",false)
  ...
  boolean acceptDeviceEvent(String device,String command,String cycle,int revision,int seq,String eventType){
    ...
    if(current&&"complete".equals(eventType))putDayRecord(edit,"COMPLETED","",recoveryCard()).putBoolean("completed",true).putString("phase",PHASE_DONE);
    else if(current)putDayRecord(edit,completed()?"COMPLETED_SEALED":"SEALED_UNFINISHED",stopNote(),recoveryCard()).putString("phase",PHASE_SEALED);
    return commit(edit);
  }
  ...
  boolean advanceDemoDay(){if(!isSimulated())return false;boolean wasDone=completed();SharedPreferences.Editor edit=recordConfirmedSnapshot(prefs.edit());if(day()>=10)return commit(edit.putString("phase",PHASE_REVIEW));return commit(edit.putInt("demo_day",day()+1).putBoolean("completed",false).putBoolean("device_saved",false).putString("phase",wasDone?PHASE_NEXT:PHASE_RESUME));}
  boolean refreshRealDate(){
    if(isSimulated()||!hasCycle())return true;
    try{ZoneId zone=ZoneId.of(text("tz"));long slot=ChronoUnit.DAYS.between(LocalDate.parse(text("cycle_start_date")),LocalDate.now(zone))+1;if(slot<=day())return true;boolean wasDone=completed();SharedPreferences.Editor edit=recordConfirmedSnapshot(prefs.edit());if(slot>10)return commit(edit.putString("phase",PHASE_REVIEW));return commit(edit.putInt("day_index",(int)slot).putBoolean("completed",false).putBoolean("device_saved",false).putString("phase",wasDone?PHASE_NEXT:PHASE_RESUME));}catch(Exception error){return false;}
  }
  boolean resumeExisting(String doneWhen){return !doneWhen.trim().isEmpty()&&commit(prefs.edit().putString("action",recoveryCard()).putString("done",doneWhen.trim()).putString("phase",PHASE_ACTIVE).putInt("revision",prefs.getInt("revision",1)+1).putString("command_id",UUID.randomUUID().toString()).putBoolean("device_saved",false));}
  ...
  String dayStatus(int day){return prefs.getString("day_"+day+"_status","");}
  String reviewSummary(){StringBuilder result=new StringBuilder();for(int day=1;day<=10;day++){String prefix="day_"+day+"_";String status=prefs.getString(prefix+"status","");result.append("第 ").append(day).append(" 天 · ").append(statusLabel(status)).append('\n');String date=prefs.getString(prefix+"date","");if(!date.isEmpty())result.append("日期：").append(date).append('\n');String action=prefs.getString(prefix+"action","");if(!action.isEmpty())result.append("动作：").append(action).append('\n');String done=prefs.getString(prefix+"done","");if(!done.isEmpty())result.append("完成条件：").append(done).append('\n');String note=prefs.getString(prefix+"note","");if(!note.isEmpty())result.append("停在：").append(note).append('\n');String next=prefs.getString(prefix+"next","");if(!next.isEmpty())result.append("下一步：").append(next).append('\n');result.append('\n');}return result.toString();}
  private String statusLabel(String status){if(status.isEmpty())return "无记录";if("COMPLETED_SEALED".equals(status))return "已完成并封存";if("SEALED_UNFINISHED".equals(status))return "未完成，已封存";if("UNFINISHED_UNSEALED".equals(status))return "未完成，未封存";if("COMPLETED".equals(status))return "已完成";return "已记录";}
  ...
  boolean archive(){if(dayStatus(day()).isEmpty()&&isConfirmedPhase()&&!commit(recordConfirmedSnapshot(prefs.edit())))return false;SharedPreferences.Editor edit=prefs.edit().putString("archived_summary",text("goal")+"\n\n"+reviewSummary()).putString("archived_cycle_id",text("cycle_id"));String[] fields={"status","date","action","done","note","next"};for(int day=1;day<=10;day++)for(String field:fields)edit.putString("archive_day_"+day+"_"+field,prefs.getString("day_"+day+"_"+field,""));String[] active={"goal","stuck","action","done","stop_note","recovery_card","phase","mode","cycle_id","command_id","cycle_start_date","tz","revision","demo_day","day_index","completed","device_saved","phone_saved","protocol_outbox","device_seq","device_event_history","host_seq_counter"};for(String key:active)edit.remove(key);for(int day=1;day<=10;day++)edit.remove("day_"+day+"_status").remove("day_"+day+"_date").remove("day_"+day+"_action").remove("day_"+day+"_done").remove("day_"+day+"_note").remove("day_"+day+"_next");return commit(edit);}
  private SharedPreferences.Editor recordEditor(String status,String note,String next){return putDayRecord(prefs.edit(),status,note,next);}
  private boolean isConfirmedPhase(){String current=phase();return PHASE_ACTIVE.equals(current)||PHASE_DONE.equals(current)||PHASE_SEALED.equals(current);}
  private SharedPreferences.Editor recordConfirmedSnapshot(SharedPreferences.Editor edit){if(!dayStatus(day()).isEmpty())return edit;String current=phase();if(PHASE_ACTIVE.equals(current))return putDayRecord(edit,"UNFINISHED_UNSEALED","",recoveryCard());if(PHASE_DONE.equals(current))return putDayRecord(edit,"COMPLETED","",recoveryCard());if(PHASE_SEALED.equals(current))return putDayRecord(edit,completed()?"COMPLETED_SEALED":"SEALED_UNFINISHED",stopNote(),recoveryCard());return edit;}
  private SharedPreferences.Editor putDayRecord(SharedPreferences.Editor edit,String status,String note,String next){String prefix="day_"+day()+"_";String date;try{date=LocalDate.parse(text("cycle_start_date")).plusDays(day()-1L).toString();}catch(Exception ignored){date="";}return edit.putString(prefix+"status",status).putString(prefix+"date",date).putString(prefix+"action",text("action")).putString(prefix+"done",text("done")).putString(prefix+"note",note).putString(prefix+"next",next);}
