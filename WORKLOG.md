...
## 2026-09-22｜串口背压/重连修复

- 已复现原版5次开关+8秒不读后再次开口零字节；只改timeout1无效。修复单pending无期限占用、Serial布尔门控和忽略write实际长度：250ms过期/有界flush/换行重同步，主机脚本保留超时半帧、bridge统一预设DTR/RTS。
- 两轮各5次重连+不读压力+恢复读+重开，fresh request_id的hello/storage/transport全部通过，同boot无重启；最大主循环间隔21ms、累计188/mark4保留。11项测试和编译通过。详见docs/pixel-companion/TRANSPORT.md。
- 当前app09E04A4F…10B11，716256bytes，bootloader未改；真实线缆断开/实体按键在压力中联验不冒充通过。
- 接到产品新契约7f260d2：节点唯一成长，点击照料；下一步读取全文后实施，100次旧demo已被否定。美术6d5981e提供兼容新增照料帧。
