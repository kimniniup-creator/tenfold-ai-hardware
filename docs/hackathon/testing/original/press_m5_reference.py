...
p=argparse.ArgumentParser();p.add_argument('--sha',required=True);p.add_argument('--reference',required=True);p.add_argument('--output',required=True);p.add_argument('--with-press-metal',action='store_true');a=p.parse_args()
...
parts=cad.parts();parts.update({k:v for k,v in cad.alternatives().items() if k.startswith('metal')});parts.update({'hardware_'+k:v for k,v in (cad.metal_reference(True) if a.with_press_metal else cad.metal_reference()).items()})
