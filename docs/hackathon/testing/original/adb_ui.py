...
import argparse,json,pathlib,re,subprocess,xml.etree.ElementTree as ET
...
p.add_argument('--out',default='.local/android-ui');p.add_argument('action',choices=['snapshot','tap','fill']);p.add_argument('value');p.add_argument('--index',type=int,default=0)
...
if a.action=='snapshot':
 ...
 root=ET.fromstring(raw);print(json.dumps([n.attrib for n in root.iter('node') if n.get('text') or n.get('class')=='android.widget.EditText'],ensure_ascii=False))
else:
 ...
 if a.action=='tap':
  ...
  tap(ns[a.index]);adb('shell','input','text',a.value.replace(' ','%s'))
