...
public class StateProbeV5 {
  ...
  public static void main(String[] args){
    ...
    m=new Memory();c=create(m);c.advanceDemoDay();
    report("R17_ACTIVE_CROSS_DAY_LOST",m.getString("day_1_action","").isEmpty(),"confirmed ACTIVE card must be recorded on next day without claiming completion");
    c.advanceDemoDay();
    report("R17_SKIP_FABRICATES_RECORD",!m.getString("day_2_action","").isEmpty(),"unconfirmed RESUME day must stay unrecorded");
    m=new Memory();c=create(m);c.startRealFromConfirmedCard();m.edit().putString("cycle_start_date",LocalDate.now(ZoneId.of(c.text("tz"))).minusDays(1).toString()).commit();c.refreshRealDate();
    report("R17_REAL_ACTIVE_CROSS_DAY_LOST",m.getString("day_1_action","").isEmpty(),"real date rollover snapshots confirmed previous card");
    m=new Memory();c=create(m);c.startRealFromConfirmedCard();c.acceptDeviceEvent("m5sticks3-p0",c.commandId(),c.text("cycle_id"),1,1,"complete");
    report("R17_DEVICE_COMPLETE_HISTORY_LOST",m.getString("day_1_status","").isEmpty(),"device event cursor and day completion must persist together");
    m=new Memory();c=create(m);c.startRealFromConfirmedCard();c.acceptDeviceEvent("m5sticks3-p0",c.commandId(),c.text("cycle_id"),1,1,"seal");
    report("R17_DEVICE_SEAL_HISTORY_LOST",m.getString("day_1_status","").isEmpty(),"device seal must be visible in daily review");
    m=new Memory();c=create(m);m.edit().putInt("demo_day",10).commit();c.confirmNewAction("day ten active","day ten condition");c.advanceDemoDay();
    report("R17_LAST_ACTIVE_REVIEW_LOST",!c.reviewSummary().contains("day ten active"),"entering review includes confirmed last day ACTIVE card");
    m=new Memory();c=create(m);m.edit().putInt("demo_day",10).commit();c.confirmNewAction("day ten archive","day ten condition");c.archive();
    report("R17_LAST_ACTIVE_ARCHIVE_LOST",!c.archivedSummary().contains("day ten archive"),"archive retains final confirmed ACTIVE card");
  }
}
